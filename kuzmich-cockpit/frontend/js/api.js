...
const API = {
  ...
  renamePrompt:     (o, n)   => API.put('/api/prompts/' + encodeURIComponent(o) + '/rename', {new_name: n}),
  ...
  companionRenameConfig:(m, o, n)=> API.put('/api/companion/config/' + m + '/' + encodeURIComponent(o) + '/rename', {new_name: n}),
  companionCreateConfig:(m, n)   => API.put('/api/companion/config/' + m + '/' + encodeURIComponent(n), {}),
  ...
  // --- Files ---
  listFiles:    (p)      => API.get('/api/files/list?path=' + encodeURIComponent(p || '')),
  readFile:     (p)      => API.get('/api/files/read?path=' + encodeURIComponent(p)),
  saveFile:     (p, c)   => API.put('/api/files/' + encodeURIComponent(p), {content: c}),
  renameFile:   (p, n)   => API.put('/api/files/' + encodeURIComponent(p) + '/rename', {new_name: n}),
  deleteFile:   (p)      => API.del('/api/files/' + encodeURIComponent(p)),
  mkdir:        (p, n)   => API.put('/api/files/' + encodeURIComponent(p) + '/mkdir', {name: n}),
